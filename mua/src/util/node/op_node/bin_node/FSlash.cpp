...
bool FSlash::interpret(const unordered_set<InterpretType> &flags)
{
      ...
      if (gcd == 1)
      {
        if (flags.count(InterpretType::DECIMALIZE) > 0)
        {
          ...
          if (is_minus)
          {
            ...
            lhs_node = std::make_unique<UnaMinus>(node_factory, minus_tok, lhs_node);
          }
        ...
        else if (removable_right_paren)
        {
          if (is_right_minus)
          {
            Token minus_tok;
            minus_tok.type = TokenType::MINUS;
            rhs_node = std::make_unique<UnaMinus>(node_factory, minus_tok, rhs_node);
          }
      ...
      if (!lhs_node->isLeaf())
      {
        if (is_left_minus)
        {
          ...
          if (lhs_node->getType() == NodeType::BINARY_OPERATOR)
          {
            ...
            left_numerator = std::make_unique<UnaMinus>(node_factory, minus_tok, left_numerator);
      ...
      if (!rhs_node->isLeaf())
      {
        if (is_right_minus)
        {
          ...
          if (rhs_node->getType() == NodeType::BINARY_OPERATOR)
          {
            ...
            right_numerator = std::make_unique<UnaMinus>(node_factory, minus_tok, right_numerator);
        ...
        if (rhs_node->tok.type == TokenType::FSLASH)
        {
          ...
          rhs_node = std::make_unique<Paren>(node_factory, paren_tok, rhs_node);
        }
        NodeUtils::replaceNode(node_right.get(), rhs_node);
      }
      change = true;
  ...
  else if (left_numerator != nullptr && left_denominator != nullptr && rhs_node != nullptr)
  {
    ...
    unique_ptr<Node> denominator = std::make_unique<Asterisk>(node_factory, left_denominator, tok_op, rhs_node);
    Token tok_paren;
    tok_paren.type = TokenType::LPAREN;
    unique_ptr<Node> paren = std::make_unique<Paren>(node_factory, tok_paren, denominator);
    if (is_minus)
    {
      Token tok_minus;
  ...
  else if (right_numerator != nullptr && right_denominator != nullptr && lhs_node != nullptr)
  {
    Token tok_op;
    tok_op.type = TokenType::ASTERISK;
    unique_ptr<Node> numerator = std::make_unique<Asterisk>(node_factory, lhs_node, tok_op, right_denominator);
    if (is_minus)
    {
      Token tok_paren;
      tok_paren.type = TokenType::LPAREN;
      ...
      Token tok_minus;
      tok_minus.type = TokenType::MINUS;
      numerator = std::make_unique<UnaMinus>(node_factory, tok_minus, numerator);
    }
  ...
  else if (left_numerator != nullptr && left_denominator != nullptr && right_numerator != nullptr && right_denominator != nullptr)
  {
    ...
    Token tok_paren;
    tok_paren.type = TokenType::LPAREN;
    unique_ptr<Node> numerator = std::make_unique<Asterisk>(node_factory, left_numerator, tok_asterisk, right_denominator);
    numerator = std::make_unique<Paren>(node_factory, tok_paren, numerator);
    if (is_minus)
    {
      Token tok_minus;
    ...
    unique_ptr<Node> denominator = std::make_unique<Asterisk>(node_factory, left_denominator, tok_asterisk, right_numerator);
    denominator = std::make_unique<Paren>(node_factory, tok_paren, denominator);
