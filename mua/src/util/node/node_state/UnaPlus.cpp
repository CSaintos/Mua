...
bool UnaPlus::interpret(const unordered_set<InterpretType> &flags)
{
  ...
  if (!is_node_leaf)
  {
    ...
    else if (child->tok.type == TokenType::LPAREN)
    {
      ...
      if (una_op_node->node != nullptr && una_op_node->node->tok.type == TokenType::FSLASH)
      {
        ...
        {
          cout << "UnaPlus extract nodes not implemented in_child's " + TokenUtils::m_TS_map[child->tok.type] + " node_left " + TokenUtils::m_TS_map[bin_op_node->node_left->tok.type] << endl;
        }
        ...
        {
          cout << "UnaPlus extract nodes not implemented in_child's " + TokenUtils::m_TS_map[child->tok.type] + " node_right " + TokenUtils::m_TS_map[bin_op_node->node_right->tok.type] << endl;
        }
      }
    }
  }
  ...
  {
    if (child->tok.type == TokenType::DIGIT)
    {
  ...
  if (res_node != nullptr)
  {
    if (res_node->isLeaf())
    {
      ...
      NodeUtils::replaceNode(node, res_node); // This code is repetitive after the else statement
    }
    else
    {
      if (is_minus)
