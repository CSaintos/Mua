#include "Plus.hpp"

std::unique_ptr<stem::EntryPoint> getObj(void)
{
  return std::make_unique<stem::math::Plus>();
}

std::string getName()
{
  return "Algebra:Plus, version:1.0.0";
}

void stem::math::Plus::entryPoint()
{
  std::cout << "Hello Plus" << std::endl;
}

stem::Node *stem::math::Plus::evaluate(stem::OpNode *op_node)
{
  //std::cout << "Why isn't this printing" << std::endl;
  if (op_node->tok.type != stem::TokenType::PLUS) return nullptr;
  
  if (op_node->getType() == NodeType::BINARY_OPERATOR)
  {
    stem::BinOpNode *bin_op_node = dynamic_cast<stem::BinOpNode*>(op_node);

    stem::Token *left_tok = &bin_op_node->node_left->tok;
    stem::Token *right_tok = &bin_op_node->node_right->tok;
    stem::Token *result_tok = &bin_op_node->tok;
    
    if (left_tok->type == stem::TokenType::DIGIT &&
        right_tok->type == stem::TokenType::DIGIT)
    {
      double left_num = std::stod(left_tok->lexemes);
      double right_num = std::stod(right_tok->lexemes);
      double result = left_num + right_num;

      result_tok->init();
      result_tok->lexemes = std::to_string(result);
      result_tok->type = stem::TokenType::DIGIT;

      std::cout << result << std::endl;
    }
  }

  return nullptr;
}
